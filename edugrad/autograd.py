    ...
    for t0 in collect_backward_graph(tensor):
        ...
        for parent, grad in zip(t0._ctx.parents, grads):
            if grad is not None and parent.requires_grad:
                ...
                For each tensor in the graph, its gradient is calculated based on the operation that created it. This
                gradient calculation uses the gradients of the 'child' tensor (the current tensor in the loop) to
                determine the gradients of its 'parent' tensors (the tensors that contributed to its creation in the
                forward pass, next in the loop).

                If a parent tensor contributes to multiple operations (nodes in the graph), its gradient is the sum of
                gradients from each of these contributions. This summation is a key aspect of the chain rule and ensures
                that gradients accumulate correctly in cases of branched computations.
