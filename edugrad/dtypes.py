...
For simplicity we only use bool, int32 and float32. Note that after applying operations, the results are usually float32
(see `data.TensorData.elementwise()`).
...
class dtypes:
    ...
    def from_py(x) -> DType:
        return dtypes.only_float if isinstance(x, float) else dtypes.bool if isinstance(x, bool) else dtypes.only_int
