...
def _pool(
    tensor: Tensor, k_: tuple[shape_int, ...], stride: tuple[int, ...] | int, dilation: tuple[int, ...] | int
) -> Tensor:
    ...
    slc_prefix, prefix, i_ = (
        [(0, x) for x in tensor.shape[: -len(k_)]],
        tensor.shape[: -len(k_)],
        tensor.shape[-len(k_) :],
    )
    if any(k > s for k, s in zip(k_, s_)) or any(d != 1 for d in d_):
        ...
        xup = (
            tensor.reshape(*prefix, *flatten((1, i) for i in i_))
...
def conv2d(
    tensor: Tensor, weight: Tensor, bias: Tensor | None = None, groups=1, stride=1, dilation=1, padding=0
) -> Tensor:
    ...
    assert groups * cin == cin_ and len(tensor.shape) == len(
        weight.shape
...
def linear(tensor: Tensor, weight: Tensor, bias: Tensor | None = None):
    """Computes a linear transformation (y = xW + b) of the input tensor.

    If the weight tensor is one-dimensional, it performs element-wise multiplication between the input tensor and the
    weight tensor. Otherwise, it performs a dot product between the input tensor and the weight tensor.

    If a bias tensor is provided, it is added to the result of the multiplication/dot product. The bias tensor must
    match the dimensions required for broadcasting over the output tensor.
...
def binary_crossentropy(tensor: Tensor, y: Tensor) -> Tensor:
    ...
    The binary cross-entropy loss is a measure used in binary classification tasks. This function calculates the loss
    for each pair of predicted and target values, then takes the mean of these losses.
...
def binary_crossentropy_logits(tensor: Tensor, y: Tensor) -> Tensor:
    ...
    This function is similar to binary_crossentropy but takes logits as input instead of probabilities. Logits are the
    raw outputs of the last linear layer and have not been passed through a sigmoid function.
...
def sparse_categorical_crossentropy(tensor: Tensor, Y, ignore_index=-1) -> Tensor:
    ...
    This function is used when the classes are mutually exclusive (e.g., each sample belongs to exactly one class). The
    input tensor is expected to be logits, and the target Y should contain integer class indices. The function supports
    an 'ignore_index' to exclude certain targets from loss computation.

    The function converts the class indices into a one-hot encoded format internally and then computes the cross-entropy
    loss.
    ...
    y_counter = (
        tensor.arange(tensor.shape[-1], dtype=dtypes.int32, requires_grad=False)
        .unsqueeze(0)
        .expand(Y.numel(), tensor.shape[-1])
    )
    y = ((y_counter == Y.flatten().reshape(-1, 1)).where(-1.0, 0) * loss_mask.reshape(-1, 1)).reshape(
        *Y.shape, tensor.shape[-1]
    )
