...
def __getitem__(
    ...
    if (num_slices := len(count[int]) + len(count[slice]) + len(count[Tensor])) > len(tensor.shape):
        raise IndexError(f"too many indices for tensor of dimension {len(tensor.shape)}")
    ...
    valid_slices = [
        ...
        for i, (v, dim_sz) in enumerate(zip(valid_slices, tensor.shape))
    ]

    start, stop, strides = (
        zip(*y) if (y := [s.indices(dim_sz) for s, dim_sz in zip(valid_slices, tensor.shape)]) else ((), (), ())
    )
    new_slice = tuple(
    ...
    if tensors:  # Fancy/tensor indexing
        ...
        arange = [
            Tensor.arange(ret.shape[d], dtype=dtypes.int32, requires_grad=False).reshape(
...
def gather(tensor: "Tensor", idx: "Tensor", dim: int) -> "Tensor":
    ...
    assert all(
        s >= i for s, i in zip(tensor.shape, idx.shape)
    ), "all dim of idx.shape must be smaller than tensor.shape"
    if dim < 0:
        dim += tensor.ndim
    ...
    return (
        (
            (idx == Tensor.arange(tensor.shape[dim], dtype=dtypes.int32, requires_grad=False))
            * tensor.permute(*permarg)
            .shrink(tuple([*[(0, sh) for sh in idx.shape[1:-1]], (0, tensor.shape[dim])]))
            .unsqueeze(0)
        )
        .sum(-1)
